...
async function processInputOrgDetails(org_list, org_data)
{
    ...
    for(let i = 0; i < org_data.length; i++)
    {
        ...
        // Match condition
        const match_condition = org_data[i].match_condition || process.env.DEFAULT_MATCH_CONDITION;
        ...
        for(let j = 0; j < org_data[i].match_transaction_fields.length; j++)
        {
            ...
            // Hierarchy & delimiter can be blank. All other fields are mandatory
            if(!this_match_transaction_key || this_match_transaction_key == "")
            {
                common.statusMessage(_fn, "Match transaction field no.: ", j, " key is missing for org id: " + org_id + " org name: " + org_name);
                return -1;
            }
            if(!this_match_transaction_type || this_match_transaction_type == "")
            {
                common.statusMessage(_fn, "Match transaction field no.: ", j, " type is missing for org id: " + org_id + " org name: " + org_name);
                return -1;
            }
            if(!this_match_transaction_pattern || this_match_transaction_pattern == "")
            {
                common.statusMessage(_fn, "Match transaction field no.: ", j, " pattern is missing for org id: " + org_id + " org name: " + org_name);
                return -1;
            }
...
{
    // Get the function name for logging purposes
    const _fn = getCardTransactions.name;
   ...
   // Get list of transactions created since the last update. If the timestamp is invalid, use 1 week interval
    ...
    common.statusMessage(_fn, "Going to retrieve transactions ", event, " after: ", after_date_str, " for org: ", this_org.org_name);

    // Get the list of transactions
    if(await fyle_acc.card_transaction.getCardTransactions(event, after_date_str, before_date_str) < 0)
    {
        common.statusMessage(_fn, "Failed to get list of transactions for org: ", this_org.org_name);
        return -1;
    }
    ...
    common.statusMessage(_fn, "Successfully retrieved ", this_org.raw_transaction_list.length, " transactions ", event, " after: ", after_date_str, " for org: ", this_org.org_name);
...
{
    // Get the function name for logging purposes
    const _fn = identifyMatchingTransactions.name;
    ...
    for(let i = 0; i < this_org.raw_transaction_list.length; i++)
    {
        ...
        if(found_match == true)
        {
            common.statusMessage(_fn, "Card Transaction selected with ID ", this_org.raw_transaction_list[i].id,);
...
{
    // Get the function name for logging purposes
    const _fn = ignoreCardTransactions.name;
    ...
    {
        common.statusMessage(_fn, "Failed to ignore card transactions for org: " + this_org.org_name);
        return -1;
    }

    common.statusMessage(_fn, "Successfully ignored card transactions for org: " + this_org.org_name);
...
async function writeLogs(org_list, org_idx)
{
    ...
    await common.GoogleSheet_writeStructuredDataArrayToGoogleSheet(logs_folder_id, logs_file_name, sheet_name, this_org.raw_transaction_list, true, true);
    ...
    await common.GoogleSheet_writeStructuredDataArrayToGoogleSheet(logs_folder_id, logs_file_name, sheet_name, this_org.final_transaction_list, true, true);
...
{
    // Get the function name for logging purposes
    const _fn = ignore_card_transactions.name;

    common.statusMessage(_fn, " ****************** Ignore Card Transactions Start ****************** ");
    ...
    for(let i = 0; i < org_list.length; i++)
    {
        ...
        common.statusMessage(_fn, "Ignore Card Transaction for Org: ", org_list[i].org_name);
        common.statusMessage(_fn, "**********************************************************************");
        ...
        common.statusMessage(_fn, "Successfully setup Fyle account for Org: ", org_list[i].org_name, " going to get card transactions");

        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        if(await getCardTransactions(org_list, i) < 0)
        {
            common.statusMessage(_fn, "Failed to get card transactions for Org: ", org_list[i].org_name, " exiting");
            continue;
        }
        common.statusMessage(_fn, "Successfully retrieved card transactions for Org: ", org_list[i].org_name, " going to identify matching transactions next");

        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        if(await identifyMatchingTransactions(org_list, i) < 0)
        {
            common.statusMessage(_fn, "Failed to identify matching transactions for Org: ", org_list[i].org_name, " exiting");
            continue;
        }
        common.statusMessage(_fn, "Successfully identified matching transactions for Org: ", org_list[i].org_name, " going to ignore matched transactions next");

        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        if(await ignoreCardTransactions(org_list, i) < 0)
        {
            common.statusMessage(_fn, "Failed to ignore card transactions for Org: ", org_list[i].org_name, " exiting");
            continue;
        }
        common.statusMessage(_fn, "Successfully ignored card transactions for Org: ", org_list[i].org_name, " going to write out the current timestamp next");
    ...
    common.statusMessage(_fn, " ****************** Ignore Card Transactions End ****************** ");
}
