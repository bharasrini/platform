...
async function partner_success_dashboard()
{
    ...
    const fn = partner_success_dashboard.name;
    ...
    {
        common.statusMessage(fn, "Failed to build the partner lists, exiting");
        return -1;    
    }
    common.statusMessage(fn, "Finished building the Partner lists, going to create the Partner Success dashboard");

    // Write the Partner Success Dashboard
    if(await createPartnerDashboard(referral_list, reseller_list, wholesale_list) < 0)
    {
        common.statusMessage(fn, "Failed to write the Partner Success Dashboard, exiting");
        return -1;    
    }
    common.statusMessage(fn, "Finished writing the Partner Success Dashboard, exiting");
