...
async function test_readDataFromGoogleSheet()
{
    ...
    const data = await common.readDataFromGoogleSheet(spreadsheet_id, sheet_name, range);
    if(data !== null)
    {
        common.statusMessage(fn, "Data read successfully from sheet with name " + sheet_name + " in spreadsheet with ID " + spreadsheet_id);
        console.log("Data: ", data);
    }
    else common.statusMessage(fn, "Failed to read data from sheet with name " + sheet_name + " in spreadsheet with ID " + spreadsheet_id);
...
{
    // Get function name for logging
    const fn = test_copySheetInGoogleSpreadsheet.name;

    common.start_test(fn);

    const source_spreadsheet_id = "1506973nwoRbDX4GnmwfkEnpTZBhxVkDL9rNQ8YXF0O8"; // Source Spreadsheet in "Test Folder" under "Customer Success Shared Drive"
    const source_sheet_name = "source";
    const destination_spreadsheet_id = "1huCetjPJVMYOSVwXNlLqsq_Wqig9NZgYY-WOruFRBUM"; // Destination Spreadsheet in "Test Folder" under "Customer Success Shared Drive"
    const dest_sheet_name = "CopiedSheet";
    const res = await common.copyInputGoogleSheet(source_spreadsheet_id, source_sheet_name, destination_spreadsheet_id, dest_sheet_name);
    if(res >= 0) common.statusMessage(fn, "Sheet with name " + source_sheet_name + " copied successfully in spreadsheet with ID " + destination_spreadsheet_id + " with new sheet name " + dest_sheet_name);
