...
{
    // Get the function name for logging
    const fn = test_readDataFromGoogleSheet.name;
    ...
    const data = await common.GoogleSheet_readDataFromGoogleSheet(sheets, spreadsheet_id, sheet_name, null);
    common.statusMessage(fn, "Data read from sheet: ", data);
...
async function test_GoogleSheet_getNumberOfSheetsInGoogleSpreadsheet()
{
    ...
    const num_sheets = await common.GoogleSheet_getNumberOfSheetsInGoogleSpreadsheet(sheets, spreadsheet_id);
...
{
    // Get the function name for logging
    const fn = test_GoogleSheet_hideGridlinesInGoogleSheet.name;
    ...
    {
        const  sheet_id = res.properties.sheetId;
        const status = await common.GoogleSheet_hideGridlinesInGoogleSheet(sheets, spreadsheet_id, sheet_id, hide_gridlines);
        if(status >= 0) common.statusMessage(fn, "Sheet with name " + sheet_name + " gridlines hidden successfully");
        else common.statusMessage(fn, "Failed to hide gridlines for sheet with name " + sheet_name);
    }
    else
    {
        common.statusMessage(fn, "Sheet with name " + sheet_name + " not found in spreadsheet, cannot hide gridlines");
    }
...
{
    // Get the function name for logging
    const fn = test_GoogleSheet_copySheet.name;
    ...
    const sheet_res = await common.GoogleSheet_findSheetByNameInGoogleSpreadsheet(sheets, source_spreadsheet_id, source_sheet_name);
    if(!sheet_res)
    {
        common.statusMessage(fn, "Sheet with name: " , source_sheet_name , " not found in spreadsheet with ID: " , source_spreadsheet_id);
        return 0;
    }

    const sheet_id = sheet_res.properties.sheetId;

    const copy_status = await common.GoogleSheet_copySheet(sheets, source_spreadsheet_id, sheet_id, destination_spreadsheet_id, dest_sheet_name);
    if(copy_status >= 0) common.statusMessage(fn, "Sheet with name " + source_sheet_name + " copied successfully in spreadsheet with ID " + destination_spreadsheet_id + " with new sheet name " + dest_sheet_name);
    else common.statusMessage(fn, "Failed to copy sheet with name " + source_sheet_name + " in spreadsheet with ID " + destination_spreadsheet_id);
...
{
    // Get the function name for logging
    const fn = test_GoogleSheet_setColumnWidth.name;
    ...
    if(res)
    {
        ...
        const status = await common.GoogleSheet_setColumnWidthsInGoogleSheet(sheets, spreadsheet_id, sheet_id, requests);
        if(status < 0)
        {
            common.statusMessage(fn, "Failed to set column width for sheet with name " + sheet_name);
        }
        else common.statusMessage(fn, "Column width set successfully for sheet with name " + sheet_name);
            
    }
    else common.statusMessage(fn, "Sheet with name: " , sheet_name , " not found in spreadsheet with ID: " , spreadsheet_id);
...
{
    // Get the function name for logging
    const fn = test_GoogleSheet_setRowHeight.name;
    ...
    if(res)
    {
        ...
        const status = await common.GoogleSheet_setRowHeightsInGoogleSheet(sheets, spreadsheet_id, sheet_id, requests);
        if(status < 0)
        {
            common.statusMessage(fn, "Failed to set row height for sheet with name " + sheet_name);
        }
        else common.statusMessage(fn, "Row height set successfully for sheet with name " + sheet_name);
...
{
    // Get the function name for logging
    const fn = test_GoogleSheet_formatRange.name;
    ...
    if(res)
    {
        ...
        const status = await common.GoogleSheet_formatRangeInGoogleSheet(sheets, spreadsheet_id, sheet_id, format_options);
        if(status < 0)
        {
            common.statusMessage(fn, "Failed to format range for sheet with name " + sheet_name);
        }
        else common.statusMessage(fn, "Range formatted successfully for sheet with name " + sheet_name);
