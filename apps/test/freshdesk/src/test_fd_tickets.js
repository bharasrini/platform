...
async function test_fd_get_tickets()
{
    ...
    const _fn = test_fd_get_tickets.name;
    ...
    common.statusMessage(_fn, "Tickets read successfully !!!. Total tickets read: ", tickets.num_tickets);
...
{
    // Get the function name for logging
    const _fn = test_fd_get_ticket_fields.name;
    ...
    common.statusMessage(_fn, "Ticket status label for code ", status_val, " is: ", status_label);

    const status_string = "Waiting on Customer";
    const status_code = await ticket_fields.getTicketStatusCode(status_string);
    common.statusMessage(_fn, "Ticket status code for label ", status_string, " is: ", status_code);

    const source_val = 7;
    const source_label = await ticket_fields.getTicketSourceVal(source_val);
    common.statusMessage(_fn, "Ticket source label for code ", source_val, " is: ", source_label);

    const source_string = "Chat";
    const source_code = await ticket_fields.getTicketSourceCode(source_string);
    common.statusMessage(_fn, "Ticket source code for label ", source_string, " is: ", source_code);

    const priority_val = 3;
    const priority_label = await ticket_fields.getTicketPriorityVal(priority_val);
    common.statusMessage(_fn, "Ticket priority label for code ", priority_val, " is: ", priority_label);
    
    const priority_string = "Urgent";
    const priority_code = await ticket_fields.getTicketPriorityCode(priority_string);
    common.statusMessage(_fn, "Ticket priority code for label ", priority_string, " is: ", priority_code);

    common.statusMessage(_fn, "Ticket fields read successfully !!!");
...
{
    // Get the function name for logging
    const _fn = test_fd_set_ticket_data.name;

    common.start_test(_fn);

    let ret = {};
    const ticket_id = 83222; // promotions ticket
    const responder_id = 9030391174; // Jahaber Sathik
    const due_by = "2026-03-01T12:00:00Z";
    const status = 7; // Waiting on Third Party
    ...
    common.statusMessage(_fn, "Ticket fields updated successfully !!!. Updated data: ", ret);
...
{
    // Get the function name for logging
    const _fn = test_fd_tickets.name;
